...
- `CANDIDATO`: implementado y probado localmente, pero no publicado ni
  aceptado en staging/live.
...
| Bots, IA y privacidad | CANDIDATO | Hubs `d7f0c2fc4`: 10/10 focales, AVA 79/79, check, build, lint, Prettier y Gitleaks verdes. Cloud `b7b752f`: Reticulum 391 pruebas + 5 properties, 0 fallos y 3 excluidas; bot orchestrator 102/102; generador 26/26. El contrato local cubre Presence, ACK, navmesh, capacidad de chat por canal, modelo reply-only, moderacion fail-closed, `store:false`, limites y logs redactados. | Resolver `AUD-075`, fencing persistente de leases y cuarentena/aprobacion ejecutable de configuraciones heredadas; despues Actions y aceptacion de rehidratacion, 0/5/10 bots, movilidades, chat y logs en staging/live. |
...
| Spoke legacy | CANDIDATO | `docs/spoke-legacy-audit-2026-07.md`; candidato Cloud `b7b752f` con lint, 68/68 y build verdes sobre el baseline Node 16/Yarn 1. | Actions y aceptacion funcional en staging conservando proyecto, escena, navmesh, ocho `spawbot-*` y dos asientos; la modernizacion de imagen sigue siendo otro rollout. |
...
| Actions e imagenes | PENDIENTE | El baseline live anterior conserva digests aceptados; el candidato actual no se presenta como publicado. | Publicar subrepos en orden, exigir Actions verdes, capturar digests y regenerar el manifiesto estandar. |
...
| Coherencia Git/documental | CANDIDATO | El indice raiz fija Hubs `d7f0c2fc4` y Cloud `b7b752f`; la documentacion y esta matriz distinguen candidato de live, y los gates normal/full pasan sobre esos gitlinks. | Confirmar y publicar subrepos en orden, actualizar el puntero root remoto y dejar los tres arboles limpios; no confundir ese cierre Git con aceptacion live. |
