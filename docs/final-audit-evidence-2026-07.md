...
| --- | --- | --- | --- |
| Releases upstream | PROBADO | `./scripts/verify-project.sh --full` repitio `audit-upstream.sh` el 17-07-2026 sobre Hubs `d7f0c2fc4` y Cloud `b7b752f`: baselines `prod-2026-03-11` y `2.1.0`, con 0 commits estables ausentes. | Repetir solo si cambia la fuente o inmediatamente antes de construir imágenes/desplegar; los conflictos contra `upstream/master` siguen siendo una senal informativa, no una release desplegable. |
...
| Bots, IA y privacidad | INTEGRADO | Hubs `d7f0c2fc4`: 10/10 focales, AVA 79/79, check, build, lint, Prettier y Gitleaks verdes. Cloud `b7b752f`: Reticulum 391 pruebas + 5 properties, 0 fallos y 3 excluidas; bot orchestrator 102/102; generador 26/26. Hubs `#3` se fusionó como `6f1f5315696c`; Cloud `#1/#2` como `c5d39e0c930`/`2164851185da`, con CI post-merge verde. El contrato cubre Presence, ACK, navmesh, capacidad de chat por canal, modelo reply-only, moderación fail-closed, `store:false`, límites y logs redactados. | Resolver `AUD-075`--`AUD-078`; después construir imágenes por Actions y aceptar rehidratación, 0/5/10 bots, movilidades, chat y logs en staging/live. |
...
| Backup/restore/seguridad | PENDIENTE | Pasan 36 regresiones de seguridad, 142 de recuperación, Actionlint, ShellCheck 0.9/0.11, Gitleaks en root/Hubs/Cloud y `verify-project` normal y `--full`. GitHub repitió el gate raíz sobre `9d2679d` en `29621665409` y `29621667500`, ambos verdes. SC2119, SC2015 y la portabilidad GNU/BSD quedaron cubiertos por regresiones. No hubo acceso ni mutación del cluster. | Probar preflight/restore en un entorno aislado y, antes de cualquier mutación real, crear checkpoint DB+storage y completar la rotación de `AUD-065`. |
| Actions e imagenes | PENDIENTE | El CI de fuentes terminó verde y Hubs/Cloud ya están integrados en sus ramas base. El bootstrap meta-hubs `#2` pertenece a `main`, el PR raíz `#1` pasó su gate de código y el baseline live conserva sus digests. No se presenta ningún digest candidato como publicado. | Fusionar el cierre documental raíz; después construir las imágenes mediante los workflows aprobados, capturar digests y regenerar el manifiesto estándar. |
...
| Coherencia Git/documental | CANDIDATO | El índice raíz fija Hubs `d7f0c2fc4` y Cloud `b7b752f`; ambos son ancestros de las ramas base mediante `6f1f5315696c` y `2164851185da`. Los árboles están limpios, el gate normal/full y CI son verdes y el PR raíz `#1` contiene el cierre reproducible. | Fusionar el PR raíz; no confundir ese cierre Git con aceptación live. |
