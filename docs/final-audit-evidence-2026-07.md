...
| Backup/restore/seguridad | PENDIENTE | Los gates locales pasan: 36 regresiones de seguridad, 137 de recuperacion, Actionlint, ShellCheck local, Gitleaks en root/Hubs/Cloud y `verify-project` normal y `--full`. Tras fusionar el bootstrap, GitHub confirma Gitleaks `trusted-base` limpio, pero el gate raiz actual se detiene en ShellCheck `SC2119` antes de completar. No hubo acceso ni mutacion del cluster. | Corregir las llamadas sin callback y exigir Actions verde; despues probar preflight/restore en un entorno aislado y, antes de cualquier mutacion real, crear checkpoint DB+storage y completar la rotacion de `AUD-065`. |
| Actions e imagenes | PENDIENTE | Las ramas candidatas estan publicadas. Hubs `#3` esta `CLEAN` y verde contra `master`; Cloud `#1` esta `CLEAN` y verde contra `development`. El bootstrap meta-hubs `#2` se fusiono en `main` como `f79175d`; la rama raiz incorporo ese `main` como `9906cc7` y Gitleaks pasa en modo `trusted-base`. Los runs raiz `29599274001` y `29599277151` fallan unicamente en ShellCheck `SC2119` sobre `capture-instance-state.sh:26`. El baseline live anterior conserva sus digests y no se presenta ningun digest candidato como publicado. | Corregir `SC2119` y repetir `#1`; integrar Hubs en `master` y Cloud en `development` seguido de `development -> master`; despues construir las imagenes por Actions, capturar digests y regenerar el manifiesto estandar. |
...
| Coherencia Git/documental | CANDIDATO | El indice raiz fija Hubs `d7f0c2fc4` y Cloud `b7b752f`; las tres ramas candidatas estan publicadas, el bootstrap `#2` ya pertenece a `main`, los arboles estan limpios y los gates locales normal/full pasan sobre esos gitlinks. | Dejar verde el CI raiz y fusionar en orden Hubs, Cloud `development`, Cloud `development -> master` y finalmente root; no confundir ese cierre Git con aceptacion live. |
