...
| --- | --- | --- | --- |
| Releases upstream | PROBADO | `./scripts/verify-project.sh --full` repitió `audit-upstream.sh` el 18-07-2026 sobre Hubs `674ece411691` y Cloud `34d1d3a8d3cc`: baselines `prod-2026-03-11` y `2.1.0`, con 0 commits estables ausentes. | Repetir solo si cambia la fuente o inmediatamente antes de construir imágenes/desplegar; los conflictos contra `upstream/master` siguen siendo una señal informativa, no una release desplegable. |
...
| Bots, IA y privacidad | INTEGRADO | Hubs `674ece411691`: AVA 97/97, Admin lint/build y Gitleaks verdes. Cloud `34d1d3a8d3cc`: Reticulum 411 pruebas + 5 properties, migración real aislada en PostgreSQL 12/14, build y seguridad verdes. Hubs `#4` y Cloud `#3`-`#6` quedaron fusionados en sus ramas base. El contrato cubre Presence, ACK, navmesh, capacidad de chat por canal, aprobación/cuarentena exacta, modelo reply-only, moderación fail-closed, `store:false`, límites y logs redactados. | Resolver `AUD-075`, `AUD-076` y `AUD-078`; revisar/aprobar el inventario migrado de `AUD-077`; después construir imágenes por Actions y aceptar rehidratación, 0/5/10 bots, movilidades, chat y logs en staging/live. |
...
| Backup/restore/seguridad | PENDIENTE | Pasan 36 regresiones de seguridad, 142 de recuperación, Actionlint, ShellCheck 0.9/0.11, Gitleaks en root/Hubs/Cloud y `verify-project --full` sobre Hubs `674ece411691`/Cloud `34d1d3a8d3cc`. GitHub repitió el gate raíz anterior sobre `9d2679d` en `29621665409` y `29621667500`, ambos verdes. SC2119, SC2015 y la portabilidad GNU/BSD quedaron cubiertos por regresiones. No hubo acceso ni mutación del cluster. | Repetir el gate en el PR raíz; probar preflight/restore en un entorno aislado y, antes de cualquier mutación real, crear checkpoint DB+storage y completar la rotación de `AUD-065`. |
| Actions e imagenes | PENDIENTE | El CI de fuentes terminó verde y Hubs/Cloud ya están integrados en sus ramas base, incluido `AUD-077`. El bootstrap meta-hubs `#2` y el cierre raíz `#1` pertenecen a `main`; el baseline live conserva sus digests. No se presenta ningún digest candidato como publicado. | Integrar los nuevos gitlinks/documentación raíz; después resolver `AUD-075`, `AUD-076` y `AUD-078`, construir las imágenes mediante los workflows aprobados, capturar digests y regenerar el manifiesto estándar. |
| Rollout y produccion | BLOQUEADO | No hubo deploy ni cambio live. `AUD-065` exige checkpoint y rotacion tras la exposicion; `AUD-075` bloquea uso publico por compartir contenedor, UID, PID namespace y cgroup. La autoridad de leases sigue en memoria sin fencing DB. La aprobación/cuarentena de `AUD-077` ya está integrada, pero su migración e inventario aún no se han ejecutado ni revisado live. | Antes de cualquier mutacion: checkpoint DB+storage nuevo y rotacion coordinada. Antes de publico: aislamiento por runner, leases con fencing y aprobación individual del inventario redactado que produzca la migración. Solo despues: Actions, digests, `kubectl diff`, apply estandar y verificador live. |
...
| Coherencia Git/documental | PROBADO LOCALMENTE | La rama raíz fija Hubs `674ece411691` y Cloud `34d1d3a8d3cc`, ambos heads de sus ramas base, y registra `AUD-077`. `verify-project --full` terminó verde sobre esos punteros después de corregir la instalación Mix desde clone limpio y la prueba de entidades; el árbol staged pasa `git diff --check`. | Repetir el gate en CI, fusionar el PR raíz y no confundir ese cierre Git con aceptación live. |
